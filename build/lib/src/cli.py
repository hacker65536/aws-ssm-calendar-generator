...
from .error_handler import (
    ...
    AWSError, ConfigurationError, FileSystemError
)
...
def export(ctx, calendar_name: str, output: Optional[str], timezone: str, include_holidays: bool, holidays_year: Optional[int]):
    ...
    with logging_manager.monitor_operation("export_calendar", {
        ...
        try:
            ...
            # Get calendar data
            click.echo(f"Fetching calendar: {calendar_name}")
            calendar_data = ssm_client.get_change_calendar(calendar_name)
            ...
            if not output:
                ...
                output = os.path.join(
                    output_dir,
                    filename.format(calendar_name=calendar_name, date=date_str)
                )
